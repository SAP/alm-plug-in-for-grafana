...
const resOptions = [
  ...
  { label: 'Minutes', value: Resolution.Minute },
...
export class QueryEditor extends PureComponent<Props> {
  ...
  dpfAddFilter = () => {
    ...
    this.loadDPFilterValueOptions(query.dataProviderFilters[query.dataProviderFilters.length - 1].key, query.dataProviderFilters.length - 1);
    ...
    return new Promise<Array<SelectableValue<string>>>(resolve => {
      if (this.dataProviderOptions.length === 0) {
        ...
        datasource.searchDataProviders(q, query.refId).then(
          result => {
            this.dataProviderOptions = result.map(value => ({label: value.text, value: value.value, description: value.value}));
      ...
      datasource.searchDataProviderFilters(dp.value, query.refId, rfilter ? rfilter : undefined, query).then(
        result => {
          ...
          result.forEach((filter, i) => {
            ...
            if (filter.type == "attribute" || filter.type == "measure" || (filter.type === "dimension" && filter.isAttribute)) {
            ...
            if (!exist && filter.type == "measure") {
              this.dataProviderMeasuresOptions = filter.values.map(value => ({label: value.label, value: value.key}));
            }
          });
          ...
          query.dataProviderFilters.forEach((filter, i) => {
            ...
            if (filter && filter.key.value && filter.key.value != ""
            && !this.dataProviderFilterOptions.find(f => { return f.value == filter.key.value })
            && !this.dataProviderCustomFilterOptions.find(f => { return f.value == filter.key.value })) {
  ...
  loadDPFilterValueOptions = (filterKey: SelectableValue<string> = {}, i: number, filterValues: SelectableValue<string>[] = []) => {
    ...
    if (filterKey.value) {
      ...
      if (rfilter) {
        this.dataProviderFilterValueOptions[i] = rfilter.values.map(value => ({value: value.key, label: value.label}));
      }
      // Check for custom values
      filterValues.forEach(fv => {
        if ( fv.value && fv.value != ""
        && !this.dataProviderFilterValueOptions[i].find(op => { return op.value == fv.value })
        && !this.dataProviderCustomFilterValueOptions[i].find(op => { return op.value == fv.value })) {
      ...
      <>
        <div className="gf-form max-width-21">
          ...
          <div className="gf-form-switch">
            <Switch
              value={isConfig}
              onChange={this.onIsConfigChange}
              css=""
            />
          </div>
        </div>
        {isConfig ? 
          ...
          <div className="gf-form">
            ...
            <div className="gf-form-switch">
              <Switch
                value={resolution?.autoDecide}
                onChange={this.onAutoDecideChange}
                css=""
              />
            </div>
          </div>
              ...
              <span className="gf-form-label" key={i}>
                ...
                >
                  <Select 
                    ...
                    allowCustomValue
                    onCreateOption={customValue => {
                      this.dataProviderCustomFilterOptions = [...this.dataProviderCustomFilterOptions, {label: customValue, value: customValue}];
                ...
                <span
                  style={f.valuesSelected ? {} : {display: "none"}}
                >
                  {(f.key.value && this.dataProviderFiltersValues[f.key.value] && !this.dataProviderFiltersValues[f.key.value].isMultiple) ?
                    <Select
                      width={15}
                      options={[...this.dataProviderFilterValueOptions[i], ...this.dataProviderCustomFilterValueOptions[i]]}
                      value={f.values}
                      onBlur={() => {
                      ...
                      allowCustomValue
                      onCreateOption={customValue => {
                        this.dataProviderCustomFilterValueOptions[i] = [...this.dataProviderCustomFilterValueOptions[i], {label: customValue, value: customValue}];
          ...
          <div className="gf-form-inline">
            <div className="gf-form max-width-21">
              ...
              <MultiSelect
                ...
                allowCustomValue
                onCreateOption={customValue => {
                  this.dataProviderCustomDimensionOptions = [...this.dataProviderCustomDimensionOptions, { label: customValue, value: customValue }]
